#!/usr/bin/env python3
"""
Analyse EB radius-prediction residuals versus oblateness and Roche-lobe filling factor.

Expected input: CSV produced by the EB radius-prediction step, containing the original
stellar database columns plus at least:
    rad_pred, rad_sigma, rad_p16, rad_p84, rad_p02_5, rad_p97_5

Main outputs:
    EB_radius_residuals_with_oblateness.csv
...
Usage:
    python analyse_radii_eb_oblateness_residuals.py
"""
...
MIN_BIN_COUNT = 3
REGRESSION_PREDICTORS = ["log10_oblateness", "R", "Teff", "Meta", "logg", "L"]
...
REQUIRED_COLUMNS = ["R", "rad_pred", "rad_sigma", "oblateness"]
OPTIONAL_NUMERIC_COLUMNS = [
    "eR1", "eR2",
    "rad_p16", "rad_p84", "rad_p02_5", "rad_p97_5",
...
def make_plots(df: pd.DataFrame, outdir: Path) -> None:
    ...
    if "abs_frac_residual" in df.columns:
        plot_bin_tables.append(binned_scatter(
            ...
            r"$\log_{10}(o)$", r"$|(R_{pred}-R_{true})/R_{true}|$",
            "Absolute fractional radius residual vs oblateness",
    ...
    scatter_with_fit(
        ...
        "Oblateness", r"$(R_{pred}-R_{true})/R_{true}$",
        "Fractional radius residual vs oblateness",
    ...
    scatter_with_fit(
        df, "R", "frac_residual",
        r"True EB radius [$R_\odot$]", r"$(R_{pred}-R_{true})/R_{true}$",
        "Fractional radius residual vs true EB radius",
        plots / "08_fractional_residual_vs_true_radius.png",
        color_by="log10_oblateness",
    )
    ...
    for i, xcol in enumerate(["Teff", "Meta", "logg", "L", "log10_L"], start=9):
        ...
        if xcol in df.columns and df[xcol].notna().sum() >= 3:
            ...
            scatter_with_fit(
                ...
                xlabel, r"$(R_{pred}-R_{true})/R_{true}$",
                f"Fractional radius residual vs {xcol}",
...
def make_pretrim_plots(df: pd.DataFrame, outdir: Path) -> None:
    plots = outdir / "plots_before_rad_cut"
    plots.mkdir(parents=True, exist_ok=True)
    plot_bin_tables = []
    ...
    if "abs_frac_residual" in df.columns:
        plot_bin_tables.append(binned_scatter(
            df, "log10_oblateness", "abs_frac_residual",
            r"$\log_{10}(o)$", r"$|(R_{pred}-R_{true})/R_{true}|$",
            "Absolute fractional radius residual vs oblateness before radius cut",
            plots / "02c_abs_fractional_residual_vs_log10_oblateness_before_rad_cut.png",
            marker_color="red",
            bin_stat="median",
        ))
    ...
    save_binned_plot_tables(
        plot_bin_tables,
        outdir / "EB_plot_binned_residuals_vs_log10_oblateness_before_rad_cut.csv",
    )
    ...
    scatter_with_fit(
        ...
        "Oblateness", r"$(R_{pred}-R_{true})/R_{true}$",
        "Fractional radius residual vs oblateness before radius cut",
        plots / "02b_fractional_residual_vs_oblateness_before_rad_cut.png",
        color_by="fill_factor",
    )
    ...
    scatter_with_fit(
        df, "R", "frac_residual",
        r"True EB radius [$R_\odot$]", r"$(R_{pred}-R_{true})/R_{true}$",
        "Fractional radius residual vs true EB radius before radius cut",
        plots / "08_fractional_residual_vs_true_radius_before_rad_cut.png",
        color_by="log10_oblateness",
    )
    ...
    for i, xcol in enumerate(["Teff", "Meta", "logg", "L", "log10_L"], start=9):
        ...
        if xcol in df.columns and df[xcol].notna().sum() >= 3:
            ...
            scatter_with_fit(
                df, xcol, "frac_residual",
                xlabel, r"$(R_{pred}-R_{true})/R_{true}$",
                f"Fractional radius residual vs {xcol} before radius cut",
                plots / f"{i:02d}_fractional_residual_vs_{xcol}_before_rad_cut.png",
                color_by="log10_oblateness",
            )
...
def main() -> None:
    ...
    symmetric_error(df, "eR1", "eR2", "sigma_R_true")
    ...
    df["delta_R"] = df["rad_pred"] - df["R"]
    df["sigma_delta_R"] = np.sqrt(df["rad_sigma"] ** 2 + df["sigma_R_true"].fillna(0.0) ** 2)
    df["frac_residual"] = df["delta_R"] / df["R"]

    # Alternative predictive uncertainty estimated directly from posterior quantiles.
    if "rad_p16" in df.columns and "rad_p84" in df.columns:
        df["sigma_pred_q68"] = 0.5 * (df["rad_p84"] - df["rad_p16"])
        df["sigma_delta_R_q68"] = np.sqrt(df["sigma_pred_q68"] ** 2 + df["sigma_R_true"].fillna(0.0) ** 2)
        df["z_R_q68"] = df["delta_R"] / df["sigma_delta_R_q68"]
        df["ci68_contains_R_true"] = (df["R"] >= df["rad_p16"]) & (df["R"] <= df["rad_p84"])
    else:
        df["sigma_pred_q68"] = np.nan
        df["sigma_delta_R_q68"] = np.nan
        df["z_R_q68"] = np.nan
        df["ci68_contains_R_true"] = False

    if "rad_p02_5" in df.columns and "rad_p97_5" in df.columns:
        df["ci95_contains_R_true"] = (df["R"] >= df["rad_p02_5"]) & (df["R"] <= df["rad_p97_5"])
    else:
        df["ci95_contains_R_true"] = False

    # Linear uncertainty propagation for f = R_pred/R_true - 1.
    df["sigma_frac_residual"] = np.sqrt(
        (df["rad_sigma"] / df["R"]) ** 2
        + ((df["rad_pred"] * df["sigma_R_true"].fillna(0.0)) / (df["R"] ** 2)) ** 2
    )

    df["z_R"] = df["delta_R"] / df["sigma_delta_R"]

    # Credible-interval residuals, if prediction quantiles are present.
    if "rad_p16" in df.columns and "rad_p84" in df.columns:
        df["delta_R_p16"] = df["rad_p16"] - df["R"]
        df["delta_R_p84"] = df["rad_p84"] - df["R"]
        df["frac_residual_p16"] = df["delta_R_p16"] / df["R"]
        df["frac_residual_p84"] = df["delta_R_p84"] / df["R"]
    if "rad_p02_5" in df.columns and "rad_p97_5" in df.columns:
        df["delta_R_p02_5"] = df["rad_p02_5"] - df["R"]
        df["delta_R_p97_5"] = df["rad_p97_5"] - df["R"]
        df["frac_residual_p02_5"] = df["delta_R_p02_5"] / df["R"]
        df["frac_residual_p97_5"] = df["delta_R_p97_5"] / df["R"]
    ...
    core_cols = ["R", "rad_pred", "rad_sigma", "oblateness", "log10_oblateness", "delta_R", "frac_residual", "z_R"]
    clean = df[finite_mask(df, core_cols)].copy()

    clean["abs_delta_R"] = clean["delta_R"].abs()
    clean["abs_frac_residual"] = clean["frac_residual"].abs()
    clean["abs_z_R"] = clean["z_R"].abs()
    clean["flag_abs_z_gt_2"] = clean["abs_z_R"] > 2
    clean["flag_abs_z_gt_3"] = clean["abs_z_R"] > 3

    pretrim_n = len(clean)
    make_pretrim_plots(clean, outdir)

    rad_window_mask = clean["R"].between(RAD_MIN, RAD_MAX, inclusive="both")
    clean = clean[rad_window_mask].copy()
    rad_window_n = len(clean)
    ...
    if clean.empty:
        raise ValueError("No rows remain after the radius-window and robust filters.")

    # Export full residual table.
    residual_path = outdir / "EB_radius_residuals_with_oblateness.csv"
    ...
    for maybe in ["R", "Teff", "Meta", "logg", "L", "log10_L"]:
        if maybe in clean.columns:
            xcols.append(maybe)

    ycols = ["delta_R", "frac_residual", "z_R", "z_R_q68"]
    ...
    binned = (
        ...
        .agg(
            ...
            z_R_median=("z_R", "median"),
            fill_factor_median=("fill_factor", "median") if "fill_factor" in clean.columns else ("R", "median"),
        )
        .reset_index()
    )
    ...
    core_export_cols = id_cols + [
        "R", "sigma_R_true", "rad_pred", "rad_sigma", "delta_R", "frac_residual",
        "sigma_frac_residual", "z_R", "sigma_pred_q68", "z_R_q68", "ci68_contains_R_true", "ci95_contains_R_true", "oblateness", "log10_oblateness",
    ...
    clean.sort_values("abs_z_R", ascending=False)[core_export_cols].head(TOP_N).to_csv(
        outdir / "EB_top_outliers_by_abs_zR.csv", index=False
    )
    if "z_R_q68" in clean.columns:
        clean["abs_z_R_q68"] = clean["z_R_q68"].abs()
        clean.sort_values("abs_z_R_q68", ascending=False)[core_export_cols].head(TOP_N).to_csv(
            outdir / "EB_top_outliers_by_abs_zR_q68.csv", index=False
        )

    make_plots(clean, outdir)
    plot_bins_path = outdir / "EB_plot_binned_residuals_vs_log10_oblateness.csv"
    pretrim_plot_bins_path = outdir / "EB_plot_binned_residuals_vs_log10_oblateness_before_rad_cut.csv"

    # Compact text report.
    summary_lines = []
    summary_lines.append(f"Input file: {input_path}")
    summary_lines.append(f"Input rows: {len(df)}")
    summary_lines.append(f"Rows usable before radius cut: {pretrim_n}")
    summary_lines.append(f"Pre-radius-cut plots directory: {(outdir / 'plots_before_rad_cut').resolve()}")
    summary_lines.append(f"Radius window: {RAD_MIN:.3f} <= R <= {RAD_MAX:.3f} solar radii")
    summary_lines.append(f"Rows after radius window: {rad_window_n}")
    ...
    summary_lines.append("  delta_R = rad_pred - R")
    summary_lines.append("  frac_residual = (rad_pred - R) / R")
    summary_lines.append("  sigma_delta_R = sqrt(rad_sigma^2 + sigma_R_true^2)")
    summary_lines.append("  z_R = delta_R / sigma_delta_R")
    summary_lines.append("")
    for col in ["delta_R", "frac_residual", "z_R", "z_R_q68", "oblateness", "fill_factor", "sigma_pred_q68", "rad_sigma"]:
    ...
    summary_lines.append("")
    if "ci68_contains_R_true" in clean.columns:
        summary_lines.append(f"68% interval coverage: {clean['ci68_contains_R_true'].mean():.3f}")
    if "ci95_contains_R_true" in clean.columns:
        summary_lines.append(f"95% interval coverage: {clean['ci95_contains_R_true'].mean():.3f}")
    summary_lines.append("")
    ...
    summary_lines.append(f"  Pre-radius-cut plot bins: {pretrim_plot_bins_path.resolve()}")
    ...
    print(f"Pre-radius-cut plot-bin table: {pretrim_plot_bins_path.resolve()}")
    print(f"Plots directory: {(outdir / 'plots').resolve()}")
    print(f"Pre-radius-cut plots directory: {(outdir / 'plots_before_rad_cut').resolve()}")
    print(f"Usable rows before radius cut: {pretrim_n} / {len(df)}")
    print(f"Rows after {RAD_MIN:.3f}-{RAD_MAX:.3f} solar-radius cut: {rad_window_n}")
    if ROBUST:
